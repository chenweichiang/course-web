...
export const CREATURE = {
  intro:
    '把你的生物想成住在一個養殖箱裡，箱子裡的條件由你決定，但每一條都要有根據。順序很重要，年代決定環境，環境決定牠怎麼生存、住在哪、怎麼繁殖，先畫出一隻很酷的生物再回頭硬湊牠住哪裡，是最常見的死法。五個問題的答案先記在 m1-world/ 的筆記，到 M2 再展開成物種設定書。',
  box: [
    {
      k: '年代',
...
export const METHOD_RHYTHM = [
  ...
  '報告站（M1、M2、M3、M6）每個人都要上台簡報：針對該站進度講清楚你做了什麼、為什麼這樣選、卡過哪裡怎麼解，三分鐘、做重點不做逐字稿，簡報檔放進該站資料夾一起 push。M1 世界分享形式不限，簡報、草圖、筆記、照片都可以。',
]

// 學期節奏：每週四上課，16 次（依本學期行事曆排定）
export const CALENDAR = {
  intro:
    '每週四上課，共 16 次，從 9/10 到 12/24，每次三小時。課程仍以里程碑為主，作品在課外推進，課堂拿來討論與解決問題，這張表告訴你每次上課討論什麼、下次上課前要推進到哪，全班同步的報告站有四個：10/1 世界分享、10/15 物種簡報、11/12 物種發表會、12/24 開園。期末考週（12/21 到 12/24）本課不考試，開園展演就排在 12/24 那堂課。',
  offdays:
...
export const WEEKS = [
  ...
  },
  {
    ...
    after: '寫出世界的一句話定案，想清楚為什麼選這個世界、哪些部分是你自己想出來的，準備 10/1 世界分享要用的東西，簡報、草圖、筆記都可以。',
  },
  {
    w: 4, date: '2026-10-01', m: 'M1', station: '世界分享', title: '世界分享與確認',
    bring: [
      '做：世界分享要用的東西，形式不限（簡報、草圖、筆記、照片都可以），放進 m1-world/，不用交完整的研究報告',
      '想：你的世界哪些部分是你自己想出來的，查資料或問 AI 的過程中，哪個發現改變了你原本的想法',
    ...
    inClass: [
      '每人約三分鐘分享自己的世界，台下同學針對根據提問',
      '老師當場跟你聊：哪些是你自己的想法、根據從哪裡來',
...
export const MILESTONE_GUIDE = [
  {
    id: 'M0', name: '裝備', flag: null, when: '第 1–2 週（9/10、9/17）', due: '先不收，什麼時候收另行公告',
  ...
  {
    id: 'M1', name: '世界', flag: '確認', when: '第 3–4 週（9/24、10/1），第 2 週先寫世界候選，10/1 世界分享與確認', due: '10/1（四）上課分享，不收完整研究報告',
    goal: '透過研究先行迴圈的第一次完整實戰，把「沒有人類之後」從想像變成你自己的、有根據的世界，你的物種能不能成立，全看這一站打的地基。',
    steps: [
      '9/24 上課帶著你手上的材料來跟老師討論：三個世界候選、查到的資料、做到一半的簡報都可以。材料不強制上傳，建議先丟進 m1-world/notes.md，10/1 分享時直接用得上。',
      ...
      '查證幾個關鍵事實，例如「混凝土多久會崩解」「輻射區的真實生態（車諾比案例）」，回到原始出處，把連結存下來，分享時說得出根據。',
      '用一句話定案：「我的世界是人類消失後＿＿年的＿＿，這裡最大的生存挑戰是＿＿。」',
      '想寫成完整的世界觀研究報告也可以，不強制，建議結構：時間點與棲地（對應養殖箱的年代）、環境條件表（對應環境）、三個查證過的事實與來源、對物種設計的啟示（先寫下生存、棲息、繁殖的初步想法），放進 m1-world/ 並 push。',
      '10/1 世界分享約三分鐘，形式不限，簡報、草圖、筆記、照片都可以。重點是你自己的想法：你的世界是什麼樣子（人類消失多久之後、在哪裡、最大的生存挑戰是什麼）、為什麼選這個世界、哪些部分是你自己想出來的，以及查資料或問 AI 的過程中，哪個發現改變了你原本的想法。講不清楚的地方，就是還沒想透的地方。',
      '分享完老師會跟你聊，確認這個世界站得住就開始設計物種，分享用的東西順手放進 m1-world/。',
    ],
    checks: [
      '9/24 已經跟老師討論過材料或簡報方向',
      '有明確的時間點與棲地',
      '關鍵事實說得出查證出處',
      '講得出為什麼選這個世界、哪些部分是你自己想出來的',
      '10/1 世界分享完成，分享用的東西已放進 m1-world/',
      '用了 AI 的地方記進 m1-world/README.md 的揭露欄',
      '確認通過',
    ],
...
└── m6-zoo/`,
  rows: [
    { f: 'm0-outfit', m: 'M0 裝備', due: '先不收', talk: '' },
    { f: 'm1-world', m: 'M1 世界', due: '10/1', talk: '世界分享' },
  ...
  notes: [
    ...
    '報告站的簡報一律轉 PDF，檔名照「站名-簡報.pdf」，例如 m2-species/m2-簡報.pdf；M1 世界分享形式不限。',
...
export const SUBMIT_STEPS = [
  ...
  { t: '交付放進里程碑資料夾', d: '資料夾名稱照這七個：m0-outfit、m1-world、m2-species、m3-creature、m4-population、m5-habitat、m6-zoo。每個資料夾放四件套：程式碼、截圖或影片、README.md（反思與 AI 揭露欄），報告站（M2、M3、M6）另加簡報 PDF，M1 世界分享形式不限。不會用終端機也沒關係，GitHub Desktop 或網頁上傳都可以。' },
  { t: '截止前交上去', d: '各站截止：M2 10/15、M3 11/12、M4 12/3、M5 12/17，都是當天上課前（15:30）。M1 在 10/1 上課分享自己的世界，形式不限，不收完整的研究報告；M0 先不收，什麼時候收另行公告。M6 在 12/24 開園前交作品與導覽簡報，反思與 AI 揭露當天 23:59 前補齊。commit 的時間戳就是交付紀錄，你的每一步嘗試都是過程的證據。' },
