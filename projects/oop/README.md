...
- 延伸閱讀單一來源＝`src/readings.js`（key＝section id 或 M0–M6；每筆 2026-09-17 實際打開核對過，改連結後要重新檢查）
- 週進度＝`src/data.js` 的 `WEEKS`（每週四，9/10–12/24）；agy 安裝說明＝`AGY_INSTALL`；想你的生物（養殖箱）＝`CREATURE`
